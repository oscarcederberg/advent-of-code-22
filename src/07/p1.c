...
size_t calculate_total_size_to_free(struct dir_t *current, size_t *freeable) {
    size_t i;
    size_t dir_size = 0;
    ...
    for ( i = 0; i < current->dir_count; i++) {
        dir_size += calculate_total_size_to_free(current->dirs[i], freeable);
    }
...
int main(void) {
    ...
    size_t sum = 0;
    calculate_total_size_to_free(root, &sum);
    printf("there is a freeable size of %lu\n", sum);
    free_dirs(root);
}